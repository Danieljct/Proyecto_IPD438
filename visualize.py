import pandas as pd
import matplotlib.pyplot as plt
import seaborn as sns
import sys
import os

# Archivo de entrada
INPUT_FILE = "benchmark_results.csv"
# Archivo de salida
OUTPUT_PLOT_FILE = "benchmark_graphs.png"

def analyze_and_plot(df):
    """
    Analiza el DataFrame y genera los 4 gráficos de precisión.
    """
    print("--- Análisis de Resultados del Benchmark ---")
    
    # Agrupar por algoritmo y memoria, calculando la media de las métricas
    # Esto promedia los resultados de todos los flujos y todos los tiempos
    df_grouped = df.groupby(['algorithm', 'memory_kb']).agg({
        'are': 'mean',
        'cosine_sim': 'mean',
        'euclidean_dist': 'mean',
        'energy_sim': 'mean'
    }).reset_index()
    
    print("Resultados promediados:")
    print(df_grouped)
    print("------------------------------------------")

    # --- Generar los 4 Gráficos (Cuadrícula 2x2) ---
    
    # Configurar el estilo de Seaborn
    sns.set_theme(style="whitegrid")
    
    fig, axes = plt.subplots(2, 2, figsize=(16, 12))
    fig.suptitle('Análisis de Precisión de Algoritmos de Medición vs. Memoria', fontsize=20, y=1.03)

    # Paleta de colores para los algoritmos
    palette = sns.color_palette("deep", len(df_grouped['algorithm'].unique()))

    # Gráfico 1: Distancia Euclidiana (a)
    ax1 = axes[0, 0]
    sns.lineplot(data=df_grouped, x='memory_kb', y='euclidean_dist', hue='algorithm', 
                 style='algorithm', markers=True, dashes=False, ax=ax1, palette=palette, legend='full')
    ax1.set_title('Distancia Euclidiana (Menor es mejor)')
    ax1.set_xlabel('Memoria (KB)')
    ax1.set_ylabel('Distancia Euclidiana')

    # Gráfico 2: ARE (b)
    ax2 = axes[0, 1]
    sns.lineplot(data=df_grouped, x='memory_kb', y='are', hue='algorithm', 
                 style='algorithm', markers=True, dashes=False, ax=ax2, palette=palette, legend=False)
    ax2.set_title('Error Relativo Promedio (ARE) (Menor es mejor)')
    ax2.set_xlabel('Memoria (KB)')
    ax2.set_ylabel('ARE')
    ax2.set_yscale('log') # El error suele verse en escala logarítmica

    # Gráfico 3: Similitud Coseno (c)
    ax3 = axes[1, 0]
    sns.lineplot(data=df_grouped, x='memory_kb', y='cosine_sim', hue='algorithm', 
                 style='algorithm', markers=True, dashes=False, ax=ax3, palette=palette, legend=False)
    ax3.set_title('Similitud Coseno (Mayor es mejor)')
    ax3.set_xlabel('Memoria (KB)')
    ax3.set_ylabel('Similitud Coseno')
    ax3.set_ylim(0.8, 1.01) # Enfocar en la parte alta

    # Gráfico 4: Similitud de Energía (d)
    ax4 = axes[1, 1]
    sns.lineplot(data=df_grouped, x='memory_kb', y='energy_sim', hue='algorithm', 
                 style='algorithm', markers=True, dashes=False, ax=ax4, palette=palette, legend=False)
    ax4.set_title('Similitud de Energía (Más cercano a 1 es mejor)')
    ax4.set_xlabel('Memoria (KB)')
    ax4.set_ylabel('Similitud de Energía')
    ax4.set_ylim(0.8, 1.01) # Enfocar en la parte alta

    # Mover la leyenda fuera de los gráficos
    handles, labels = ax1.get_legend_handles_labels()
    fig.legend(handles, labels, loc='center right', bbox_to_anchor=(1.15, 0.5), title="Algoritmo")
    ax1.get_legend().remove() # Quitar la leyenda individual

    plt.tight_layout(rect=[0, 0.03, 1, 0.95])
    
    # Guardar el gráfico en un archivo
    plt.savefig(OUTPUT_PLOT_FILE, bbox_inches='tight')
    print(f"Gráficos de análisis guardados en: {OUTPUT_PLOT_FILE}")
    
    plt.show() # Descomenta si quieres que se muestre en pantalla

def main():
    if not os.path.exists(INPUT_FILE):
        print(f"Error: No se encontró el archivo '{INPUT_FILE}'.")
        print("Asegúrate de ejecutar el script 'run_benchmark_sweep.sh' primero.")
        sys.exit(1)
        
    try:
        df = pd.read_csv(INPUT_FILE)
        print(f"Datos cargados exitosamente desde {INPUT_FILE}")
        if df.empty:
            print("Error: El archivo de datos está vacío.")
            sys.exit(1)
        analyze_and_plot(df)
    except Exception as e:
        print(f"Ocurrió un error al procesar el archivo: {e}")
        sys.exit(1)

if __name__ == "__main__":
    main()
